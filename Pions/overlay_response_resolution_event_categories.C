...
int point_col[5] = {kBlack, kBlue, kRed, kBlue, kRed};
...
void overlay_response(const char* method="", bool save_ = false){
  ...
  if(!strcmp(method,"fixed")) {
    sprintf(tex_method,"Fixed weights");
    y_range[0] = 0.5; y_range[1] = 1.5;
    ratio_range[0] = 0.9; ratio_range[1] = 1.15;
    sprintf(canvas_savename,"response_event_categories_fixed_weights");

  }
  else if(!strcmp(method,"Ebeam")) {
    sprintf(tex_method,"Energy dependent weights");
    y_range[0] = 0.5; y_range[1] = 1.5;
    ratio_range[0] = 0.9; ratio_range[1] = 1.15;
    sprintf(canvas_savename,"response_event_categories_Edep_weights");
  }
  else {
  ...
  legName.push_back("Inclusive");
  legName.push_back("Showering in CE-E");
  legName.push_back("MIPs in CE-E");
  ...
  sprintf(temp_txt,"./txt_files/response_resolution/data/response_data_chi2method%s_all_gaus.txt",method);
  txt_fileName.push_back(temp_txt);
  sprintf(temp_txt,"./txt_files/response_resolution/data/response_data_chi2method%s_EE_gaus.txt",method);
  txt_fileName.push_back(temp_txt);
  sprintf(temp_txt,"./txt_files/response_resolution/data/response_data_chi2method%s_FH_gaus.txt",method);
  txt_fileName.push_back(temp_txt);
...
void overlay_resolution(bool doResoFit = false, const char* method="", bool save_ = false){
  ...
  if(!strcmp(method,"fixed")) {
    sprintf(tex_method,"Fixed weights");
    sprintf(canvas_savename,"resolution_event_categories_fixed_weights");
  }
  else if(!strcmp(method,"Ebeam")) {
    sprintf(tex_method,"Energy dependent weights");
    sprintf(canvas_savename,"resolution_event_categories_Edep_weights");
  }
  else {
    cout<<"method: [fixed or Ebeam]"<<endl;
    return;
  ...
  sprintf(temp_txt,"./txt_files/response_resolution/data/resolution_data_chi2method%s_all_gaus.txt",method);
  txt_fileName.push_back(temp_txt);
  sprintf(temp_txt,"./txt_files/response_resolution/data/resolution_data_chi2method%s_EE_gaus.txt",method);
  txt_fileName.push_back(temp_txt);
  sprintf(temp_txt,"./txt_files/response_resolution/data/resolution_data_chi2method%s_FH_gaus.txt",method);
  txt_fileName.push_back(temp_txt);
...
void run_all_resolution(bool save_ = false) {
  gROOT->SetBatch(1);
  overlay_resolution(1,"fixed",save_);
  overlay_resolution(1,"Ebeam",save_);
...
void run_all_response(bool save_ = false) {
  gROOT->SetBatch(1);
  overlay_response("fixed",save_);
  overlay_response("Ebeam",save_);
  
}

// void run_all_response_comparison(bool save_ = false) {
//   gROOT->SetBatch(1);
//   std::string methods[3] = {"gaus","histmean","trunc"};
//   for(int i = 0; i < 1; i++) {
