...
void overlay_response(const char* shower="EE", const char* method="gaus", bool save_ = false){
  ...
  float ratio_range[2];
  if(!strcmp(shower,"EE")) {
    if(!strcmp(method,"gaus")) { y_range[0] = 0.9; y_range[1] = 1.2;}
  ...
  sprintf(temp_txt,"%s/txt_files/response_resolution/data/response_data_chi2methodEbeam_%s_%s.txt",base_dir,shower,method);
  txt_fileName.push_back(temp_txt);
  sprintf(temp_txt,"%s/txt_files/response_resolution/sim/response_FTFP_BERT_EMN_chi2methodEbeam_%s_%s.txt",base_dir,shower,method);
  txt_fileName.push_back(temp_txt);
  sprintf(temp_txt,"%s/txt_files/response_resolution/sim/response_QGSP_FTFP_BERT_EMN_chi2methodEbeam_%s_%s.txt",base_dir,shower,method);
  txt_fileName.push_back(temp_txt);
  ...
  if(save_){
    sprintf(save_name,"%s/response_%s_%s_ratio_datasim_Ebeam_MCrescaled.png",can_t,shower,method);
    mycanvas->SaveAs(save_name);
    sprintf(save_name,"%s/pdf/response_%s_%s_ratio_datasim_Ebeam_MCrescaled.pdf",can_t,shower,method);
    mycanvas->SaveAs(save_name);
...
void overlay_resolution(bool doResoFit = false, const char* shower="EE",const char* method="gaus", bool save_ = false){
  ...
  sprintf(temp_txt,"%s/txt_files/response_resolution/data/resolution_data_chi2methodEbeam_%s_%s.txt",base_dir,shower,method);
  txt_fileName.push_back(temp_txt);
  sprintf(temp_txt,"%s/txt_files/response_resolution/sim/resolution_FTFP_BERT_EMN_chi2methodEbeam_%s_%s.txt",base_dir,shower,method);
  txt_fileName.push_back(temp_txt);
  sprintf(temp_txt,"%s/txt_files/response_resolution/sim/resolution_QGSP_FTFP_BERT_EMN_chi2methodEbeam_%s_%s.txt",base_dir,shower,method);
  txt_fileName.push_back(temp_txt);
  ...
  sprintf(can_t,"./plots/response_resolution");
  if(save_){
    sprintf(save_name,"%s/resolution_%s_%s_datasim_Ebeam_MCrescaled.png",can_t,shower,method);
    mycanvas->SaveAs(save_name);
    sprintf(save_name,"%s/pdf/resolution_%s_%s_datasim_Ebeam_MCrescaled.pdf",can_t,shower,method);
    mycanvas->SaveAs(save_name);
    // sprintf(save_name,"%s.jpg",can_t);
    // mycanvas->SaveAs(save_name);
    // sprintf(save_name,"%s.gif",can_t);
