...
//TString legend_text[9] = {"All events","Channel Masking","Noise rejection","Track quality cut","Muon veto","Pre-shower reject","Track-window selection","dR < 18.0cm","dR < 20.0cm"};
//TString legend_text[9] = {"All events","CM","CM+NR","CM+NR+TQ","CM+NR+TQ+MV","CM+NR+TQ+MV+PSRej","CM+NR+TQ+MV+PSRej+TrkWindow","dR < 18.0cm","dR < 20.0cm"};
TString legend_text[9] = {"All events","Mask","Mask+NoiseRej","Mask+NoiseRej+TrkQlty","Mask+NoiseRej+TrkQlty+muVeto","Mask+NoiseRej+TrkQlty+muVeto+PSRej","Mask+NoiseRej+TrkQlty+muVeto+PSRej+TrkWindow","dR < 18.0cm","dR < 20.0cm"};
...
//int line_color[10] = {kBlack, kBlue, kRed, kGreen+2, kMagenta, kCyan+2 , kBlue + 2 , kCyan + 1 , kGreen + 3 , kOrange+1};
int line_color[7] = {kBlue-10, kAzure-8, kBlue-7, kBlue-4, kBlue, kBlue+2 , kRed};
...
void generate_1Dplot(vector<TH1F*> hist, char const *tag_name="", int energy=-1, char const *leg_head="",
  ...
  double y = 0.90;
  TLegend *legend;
  legend = new TLegend(0.15,0.95,0.99,0.70);  
  //legend = new TLegend(0.55,0.95,0.99,0.50);  
  ...
  for(int i = 0; i < (int)hist.size(); i++) {
    ...
    if(ymax < hist.at(i)->GetMaximum()) ymax = hist.at(i)->GetMaximum();
  ...
  for(int i = 0; i < (int)hist.size(); i++) {
  	if(!log_flag) hist.at(i)->GetYaxis()->SetRangeUser(0.0,ymax*1.5);
...
void run_one(int energy, bool normalize=false, bool log_flag = false,
  ...
  sprintf(xtit,"Energy sum in HGCAL + AHCAL [MIPs]");

  vector<TString> hist_list;
  sprintf(hist_dir,"transverse_inclusive/transverse_gev_summed/transverse_%s_gev",SS_text);
  sprintf(hist_name_1,"h_rechit_energy_raw_all");
  hist_list.push_back(hist_name_1);
  sprintf(hist_name_1,"h_rechit_energy_ChannelMask_all");
  hist_list.push_back(hist_name_1);
  sprintf(hist_name_1,"h_rechit_energy_NoiseRejection_all");
  hist_list.push_back(hist_name_1);
  sprintf(hist_name_1,"h_rechit_energy_TrackQuality_all");
  hist_list.push_back(hist_name_1);
  sprintf(hist_name_1,"h_rechit_energy_MuonVeto_all");
  hist_list.push_back(hist_name_1);
  sprintf(hist_name_1,"h_rechit_energy_PreShowerReject_all");
  hist_list.push_back(hist_name_1);
  sprintf(hist_name_1,"h_rechit_energy_TrackWindow_all");
  hist_list.push_back(hist_name_1);
  ...
  for(int i = 0; i < hist_list.size(); i++) {
    ...
    if(!hist_temp) {
  	...
  	hist_temp->SetName(legend_text[i].Data());
  ...
  sprintf(tag,"CleaningCutFlow_%03dGeV_CleaningCuts",energy);
...
void run_two(int energy, bool save_ = 0) {
  run_one(energy,0,0,1,save_);
}

void run_all(bool save_ = 0) {
	gROOT->SetBatch(1);
	run_two(20,save_);
	run_two(50,save_);
	run_two(100,save_);
