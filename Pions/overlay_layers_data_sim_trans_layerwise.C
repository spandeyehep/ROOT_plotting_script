...
void generate_1Dplot(vector<TH1F*> hist,char const *tag_name="", int energy=-1, int layer=1,char const *leg_head="",
  ...
  textOnTop->DrawLatexNDC(0.7,0.92,en_lat);
  ...
  textOnTop->DrawLatexNDC(0.50,0.82,en_lat1);
  ...
  textOnTop->SetTextSize(0.07);
  textOnTop->DrawLatexNDC(0.55,0.75,en_lat1);
  ...
  textOnTop->DrawLatexNDC(0.58,0.68,en_lat1);
  ...
  if(save_canvas) {
    ...
    sprintf(canvas_name,"./linear_x/%03dGeV/%s/%s_L%02d_%dgev.png",energy,SS_text,SS_text,layer,energy);
