...
fnameOct = "/home/shubham/work/HGCAL/CERNTB/oct2018TB/analysis_code_prompt_reco/MIP_calib/txt_file/v7/calibration_all_FH_v7_plotting_compatible.txt"
...
octTB = np.loadtxt(fnameOct)    ## for rechit (x,y) position
...
FHModules = [96,106,94,102,139,131,119,45,64,62,54,43]
...
def run_one_layer(Module,octTB):
    print "INFO: Working on Module:",Module
    print "INFO: OctTB"
    
    ################## First work on (x,y) co-ordinates of rechits ############
    octTB = (octTB[octTB[:,0] == Module])   ## Takes columns for given layer
    if(not len(octTB)):
        print "Module %d Not Found in OctTB detector configuration."%Module
        return
    chip_l_o = octTB[:,1]  ## rechit_x
    channel_l_o = octTB[:,2]
    mip_l_o = octTB[:,3]
    chi2ndf_l_o = octTB[:,4]
    mip_un_l_o = octTB[:,5]
    ...
    # chip_l_j = juneTB[:,1]  ## rechit_x
    # channel_l_j = juneTB[:,2]
    # mip_l_j = juneTB[:,3]
    # chi2ndf_l_j = juneTB[:,4]
    # mip_un_l_j = juneTB[:,5]
    ...
    # legend_elements  = [ Line2D(poinx,poiny,color='w',markerfacecolor='blue',marker="o",label="OctTB",markersize=10),
    #     Line2D(poinx,poiny,color='w',markerfacecolor='red',marker="v",label="JuneTB",markersize=10)]
    ...
    # p1 = [64,20]
    # p2 = [64,50]
    oct_module = getIndex(FHModules,Module)
    layer_ = "Central module FH Layer = "+str(oct_module+1)
    plt.text(40,75,layer_,fontsize=20,color="red")
    ...
    canvas_name = "./FH_modules/"+str(Module)+'.png'
...
i = start-1
n = end
#newModules = [82,72,142,143,144,145,96,106,94,102,139,131,119,45,64,62,54,43]
#FHModules = [96,106,94,102,139,131,119,45,64,62,54,43]
#while (i < end):
for i in FHModules:
